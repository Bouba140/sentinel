...
& dotnet pack "./Sentinel/Sentinel.csproj" -p:PackageVersion=$version
$package = "$basePath\$productName.$version.nupkg"
Write-Host "package $package"

if ( Test-Path $package ) {
    Write-Host "Processing nuget package: $package"
    Write-Host "Packed into $package"
    $releasesLocation = "$basePath\net10.0-windows\Releases"
